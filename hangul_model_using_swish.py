...
def main(label_file, tfrecords_dir, model_output_dir, num_train_steps):
    ...
    h_conv1 = tf.nn.swish(tf.nn.conv2d(x_image, W_conv1, strides=[1, 1, 1, 1], padding='SAME') + b_conv1)
    ...
    h_conv2 = tf.nn.swish(tf.nn.conv2d(h_pool1, W_conv2, strides=[1, 1, 1, 1], padding='SAME') + b_conv2)
    ...
    h_conv3 = tf.nn.swish(tf.nn.conv2d(h_pool2, W_conv3, strides=[1, 2, 2, 1], padding='SAME') + b_conv3)

    # 네번째 convolutional layer
    W_conv4 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 128], stddev=5e-2))
    b_conv4 = tf.Variable(tf.constant(0.1, shape=[128])) 
    h_conv4 = tf.nn.swish(tf.nn.conv2d(h_conv3, W_conv4, strides=[1, 1, 1, 1], padding='SAME') + b_conv4)

    # 다섯번째 convolutional layer
    W_conv5 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 128], stddev=5e-2))
    b_conv5 = tf.Variable(tf.constant(0.1, shape=[128]))
    h_conv5 = tf.nn.swish(tf.nn.conv2d(h_conv4, W_conv5, strides=[1, 1, 1, 1], padding='SAME') + b_conv5)

    # Fully connected layer.
    h_pool_flat = tf.reshape(h_conv5, [-1, 4*4*128])
    W_fc1 = weight_variable([4*4*128, 1024])
    b_fc1 = bias_variable([1024])
    h_fc1 = tf.nn.swish(tf.matmul(h_pool_flat, W_fc1) + b_fc1)
